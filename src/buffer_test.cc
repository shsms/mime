...
TEST_F(BufferTest, Getters) {
    mime::buffer_text t{ascii, ascii.contents};
    EXPECT_EQ(t.get_buffer().contents, t.get_text());

    mime::buffer_int i{ascii, 42};
    EXPECT_EQ(i.get_buffer().contents, ascii.contents);
    EXPECT_EQ(i.get_int(), 42);

    mime::buffer_bool b{ascii, true};
    EXPECT_EQ(b.get_buffer().contents, ascii.contents);
